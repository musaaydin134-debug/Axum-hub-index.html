...
local Window = library:Window({Text = "MUSA BABA PRO 31"})

-- Sadece panelin dış çizgisine RGB ışık.
task.spawn(function()
    ...
    local main
    for _ = 1, 100 do
        main = gui:FindFirstChild("Main", true)
        if main and main:IsA("GuiObject") then break end
        task.wait(0.05)
    end
    if not main or not main:IsA("GuiObject") then return end

    local stroke = main:FindFirstChild("RGBBorder")
    if not stroke then
        stroke = Instance.new("UIStroke")
        stroke.Name = "RGBBorder"
        stroke.Thickness = 2
    ...
    while stroke.Parent do
        stroke.Color = Color3.fromHSV((os.clock() * 0.2) % 1, 1, 1)
        task.wait(0.03)
    end
end)

-- BAŞLIK: MUSA BABA PRO 31 yerine logo + AXUM HUB
task.spawn(function()
    task.wait(0.2)
    ...
    for _, obj in ipairs(gui:GetDescendants()) do
        if obj:IsA("TextLabel") and (
            obj.Text == "MUSA BABA PRO 31"
            or obj.Text == "MUSA BABA PRO 3]"
        ) then
            local parent = obj.Parent
            obj.Text = ""

            -- Eski başlık varsa tekrar ekleme.
            if parent:FindFirstChild("AXUMHeader") then
                return
            end

            local header = Instance.new("Frame")
            header.Name = "AXUMHeader"
            header.BackgroundTransparency = 1
            header.BorderSizePixel = 0
            header.AnchorPoint = obj.AnchorPoint
            header.Position = obj.Position
            header.Size = UDim2.new(0, 220, 0, math.max(obj.AbsoluteSize.Y, 32))
            header.ZIndex = obj.ZIndex + 2
            ...
            logo.ZIndex = header.ZIndex + 1

            if getcustomasset then
                logo.Image = getcustomasset("images.png")
            end

            logo.Parent = header

            -- Logonun yanında AXUM HUB
            local title = Instance.new("TextLabel")
            title.Name = "AXUMTitle"
            title.BackgroundTransparency = 1
            title.Position = UDim2.new(0, 38, 0, 0)
            title.Size = UDim2.new(1, -38, 1, 0)
            title.Font = Enum.Font.GothamBold
            title.Text = "AXUM HUB"
            title.TextColor3 = Color3.fromRGB(225, 228, 232)
            title.TextSize = 16
...
-- PANEL BOYUTU: arayüzü %25 büyüt
task.spawn(function()
    task.wait(0.15)
    local gui = library.ScreenGui or PlayerGui:FindFirstChild("EndorisFTAP") or PlayerGui:FindFirstChildWhichIsA("ScreenGui")
    if not gui then return end

    local scale = gui:FindFirstChild("PanelScale")
    if not scale then
...
-- PANEL TEMASI: gri arka planları tamamen siyaha çevir
-- Sadece GUI arka planlarını değiştirir; yazı renklerini değiştirmez.
task.spawn(function()
    task.wait(0.15)
    local gui = library.ScreenGui or PlayerGui:FindFirstChild("EndorisFTAP") or PlayerGui:FindFirstChildWhichIsA("ScreenGui")
    if not gui then return end

    local function blacken(obj)
    ...
    gui.DescendantAdded:Connect(function(obj)
        task.defer(function()
            if obj.Parent then blacken(obj) end
        end)
    end)
end)

-- Tüm toggle'ların kapalı halini görseldeki gibi gri yap.
-- Sadece ToggleFrame ve içindeki CheckIcon'a dokunur.
task.spawn(function()
    task.wait(0.2)
    local gui = library and library.ScreenGui
    if not gui then return end

    local function fixToggle(obj)
        if not obj:IsA("GuiObject") then return end

        if obj.Name == "ToggleFrame" then
            -- Kapalı toggle: gri arka parça. Açıkken yeşil rengini bozma.
            local c = obj.BackgroundColor3
            if c.R < 0.08 and c.G < 0.08 and c.B < 0.08 then
                obj.BackgroundColor3 = Color3.fromRGB(68, 68, 68)
                obj.BackgroundTransparency = 0
            end
        elseif obj.Name == "CheckIcon" then
            -- Kapalı toggle'ın içindeki küçük yuvarlak.
            local c = obj.BackgroundColor3
            if c.R < 0.08 and c.G < 0.08 and c.B < 0.08 then
...
local miscConfigSec = MainTab:Section({Text = "Config"})
...
do
    ...
    local DropdownValues = {"Spooky Candle"}
    local SelectedToy = ToyList["Spooky Candle"]

    antiKickSec:Dropdown({
        Text = "anti kick item - Spooky Candle",
        Flag = "AntiKickItemSelect",
        List = DropdownValues,
    ...
    local function SpawnAntiKickToy(ToyName)
        ...
        local started = tick()
        repeat task.wait() until spawnedObject or (tick() - started) > 2.5
    ...
    local function DestroyAntiKickItems()
        local inv = Workspace:FindFirstChild(LocalPlayer.Name .. "SpawnedInToys")
    ...
    antiKickSec:Toggle({
        ...
        Callback = function(Val)
            ...
            Settings.Anti.AntiKickItem = Val
                ...
                task.spawn(function()
                    ...
                    while AntiKickItemActive and task.wait() do
                        ...
                        if not Item or not SoundPart then
                            ...
                            Item = SpawnAntiKickToy(SelectedToy)
                            if not Item then continue end

                            SoundPart = FWD(Item, "Hitbox", 0.5)
...
local HyperRagdollActive = false
local hyperRagdollConn = nil
local hyperRagdollTask = nil

ownershipKickSec:Toggle({Text = "Ragdoll Target", Flag = "OwnershipPalletRagdoll", Default = false, Callback = function(Value)
   viewArrow.Size = UDim2.new(0, 20, 0, 14)
    viewArrow.Position = UDim2.new(0.5, 0, 0.5, 0)
    viewArrow.BackgroundTransparency = 1
    viewArrow.Image = "rbxassetid://6034818398" -- Arrow/Pointer texture ID
    viewArrow.ImageColor3 = Color3.fromRGB(255, 105, 180)
    viewArrow.ZIndex = 5
    ...
    State.miniMapRenderConn = RunService.RenderStepped:Connect(function()
        ...
        if camera then
            ...
            viewArrow.Rotation = math.deg(angle)
        end

        local now = tick()
        if now - State.miniMapLastScanTime > 0.5 or (hrp.Position - State.miniMapLastScanPos).Magnitude > 20 then
            State.miniMapLastScanTime = now
            State.miniMapLastScanPos = hrp.Position

            local filterList = {}
            if char then table.insert(filterList, char) end
            mapRp.FilterMode = Enum.RaycastFilterType.Exclude
            mapRp.FilterDescendantsInstances = filterList

            local range = State.miniMapZoom
            local step = range / gridRes

            for x = 1, gridRes do
                for y = 1, gridRes do
                    local offsetX = (x - (gridRes / 2)) * step
                    local offsetZ = (y - (gridRes / 2)) * step
                    local origin = hrp.Position + Vector3.new(offsetX, 200, offsetZ)
                    local result = Workspace:Raycast(origin, Vector3.new(0, -400, 0), mapRp)
                    local px = State.miniMapPixels[x][y]
                    if result then
                        px.BackgroundColor3 = result.Instance.Color
                    else
                        px.BackgroundColor3 = Color3.fromRGB(15, 15, 15)
                    end
                end
            end
        end
        ...
        for _, player in ipairs(Players:GetPlayers()) do
            if player ~= LocalPlayer and player.Character and player.Character:FindFirstChild("HumanoidRootPart") then
                ...
                if math.abs(diff.X) <= range/2 and math.abs(diff.Z) <= range/2 then
                    local dot = Instance.new("Frame", MapFrame)
                    dot.Size = UDim2.new(0, 6, 0, 6)
                    dot.AnchorPoint = Vector2.new(0.5, 0.5)
                    local posX = 0.5 + (diff.X / range)
                    local posZ = 0.5 + (diff.Z / range)
                    dot.Position = UDim2.new(posX, 0, posZ, 0)
                    dot.BackgroundColor3 = Color3.fromRGB(255, 50, 50)
                    dot.BorderSizePixel = 0
                    dot.ZIndex = 4
                    table.insert(State.miniMapPlayerDots, dot)
                end
            end
        end
    end)
...
return {
    ...
    BlobmanBetaFeature = BlobmanBetaFeature,
    ...
    if not target then
        warn("[Ragdoll] Select target first!")
        HyperRagdollActive = false
        Settings.Loop.palletRagdollActive = false
        return
    end

    hyperRagdollTask = task.spawn(function()
        local myChar = LP.Character or LP.CharacterAdded:Wait()
        local myHrp = myChar:FindFirstChild("HumanoidRootPart")
        if not myHrp or not HyperRagdollActive then return end

        pcall(function()
            rSpawn:InvokeServer("PalletLightBrown", myHrp.CFrame * CFrame.new(0, -10, 0), Vector3.zero)
        end)

        local toyFolder = workspace:WaitForChild(LP.Name .. "SpawnedInToys", 4)
        local pallet = toyFolder and toyFolder:WaitForChild("PalletLightBrown", 4)
        local soundPart = pallet and pallet:WaitForChild("SoundPart", 3)

        if not soundPart or not pallet or not HyperRagdollActive then
            cleanup()
            return
        end
        ...
        local flip = false
        hyperRagdollConn = RunService.Heartbeat:Connect(function()
            if not HyperRagdollActive or not pallet.Parent then
                cleanup()
                return
            end

            local currentTarget = ownershipKickTarget and Players:FindFirstChild(ownershipKickTarget)
            ...
            if tHrp and tHum and tHum.Health > 0 then
                flip = not flip
                pcall(function()
                    rOwner:FireServer(soundPart, soundPart.CFrame)
                    rDestroyLine:FireServer(soundPart)
                    rOwner:FireServer(tHrp, tHrp.CFrame)
                end)

                if flip then
                    soundPart.CFrame = tHrp.CFrame * CFrame.new(0, 1.2, 0)
                    soundPart.AssemblyLinearVelocity = Vector3.new(0, -99999, 0)
                else
                    soundPart.CFrame = tHrp.CFrame * CFrame.new(0, -1.2, 0)
                    soundPart.AssemblyLinearVelocity = Vector3.new(0, 99999, 0)
                end
            else
                soundPart.CFrame = CFrame.new(0, 1e6, 0)
                soundPart.AssemblyLinearVelocity = Vector3.zero
            end
        end)
    end)
...
-- Pallet Fling uses the shared Ownership target.
...
-- Clone Fling uses the shared Ownership target.
...
-- Glass Box Fling uses the shared Ownership target.
...
-- Self Fling uses the shared Ownership target.
...
miscOtherSec:Toggle({Text = "ESP(Avatar)", Flag = "AvatarESP", Default = false, Callback = function(v)
    Settings.Misc.nameESP = false
...
miscLagSec:Button({
    ...
    Callback = function()
        ...
        local GrabEvent = ReplicatedStorage:WaitForChild("GrabEvents"):WaitForChild("ExtendGrabLine")
        local targetBytes = math.max(1, math.floor((packetSize or 1) * 1024 * 1024))
        local chunk = "Balls Balls Balls Balls"
        local chunkLen = #chunk
        local repeatCount = math.max(updateLineAmount(), math.ceil(targetBytes / chunkLen))
        pcall(function()
            GrabEvent:FireServer(string.rep(chunk, repeatCount))
        end)
    end
})
...
-- Third Party Protection (own-spawned NinjaShuriken -> selected targets)
do
    local TPP_Protected = {}
    local TPP_AntiGrab = false
    local TPP_AntiKick = false
    local TPP_Handling = {}
    local TPP_Shurikens = {}
    local TPP_LastSpawn = {}
    local TPP_RescueCooldown = {}
    local TPP_NearDistance = 12

    local TPP_Players = game:GetService("Players")
    local TPP_RS = game:GetService("ReplicatedStorage")
    local TPP_RunService = game:GetService("RunService")
    local TPP_LP = TPP_Players.LocalPlayer

    local TPP_GrabEvents = TPP_RS:WaitForChild("GrabEvents", 5)
    local TPP_SetOwner = TPP_GrabEvents and TPP_GrabEvents:FindFirstChild("SetNetworkOwner")
    local TPP_DestroyLine = TPP_GrabEvents and TPP_GrabEvents:FindFirstChild("DestroyGrabLine")
    local TPP_EndEarly = TPP_GrabEvents and TPP_GrabEvents:FindFirstChild("EndGrabEarly")

    local TPP_MenuToys = TPP_RS:WaitForChild("MenuToys", 5)
    local TPP_SpawnToy = TPP_MenuToys and TPP_MenuToys:FindFirstChild("SpawnToyRemoteFunction")
    local TPP_DestroyToy = TPP_MenuToys and TPP_MenuToys:FindFirstChild("DestroyToy")
    local TPP_CanSpawn = TPP_LP:FindFirstChild("CanSpawnToy")
    local TPP_StickyEvent = TPP_RS:WaitForChild("PlayerEvents", 5) and TPP_RS.PlayerEvents:FindFirstChild("StickyPartEvent")

    local function TPP_GetPlayerList()
        local list = {}
        for _, plr in ipairs(TPP_Players:GetPlayers()) do
            if plr ~= TPP_LP then
                local item = plr.DisplayName .. " (@" .. plr.Name .. ")"
                if TPP_Protected[plr.Name] then
                    item = "[✓] " .. item
                end
    ...
    local function TPP_GetRoot(plr)
        local char = plr and plr.Character
        return char and (char:FindFirstChild("HumanoidRootPart") or char:FindFirstChild("Torso"))
    end

    local function TPP_GetTargetPart(plr)
        local char = plr and plr.Character
        if not char then return nil end
        -- Put the Kunai inside the torso, slightly toward the back (+Z), not on top of the head.
        return char:FindFirstChild("UpperTorso") or char:FindFirstChild("Torso") or char:FindFirstChild("HumanoidRootPart")
    end

    local function TPP_DestroyFor(plr)
        local toy = TPP_Shurikens[plr.Name]
        TPP_Shurikens[plr.Name] = nil
        if toy and toy.Parent and TPP_DestroyToy then
            pcall(function() TPP_DestroyToy:FireServer(toy) end)
        end
    end

    local function TPP_SpawnFor(plr)
        if not TPP_AntiKick or not TPP_Protected[plr.Name] then return nil end
        if not TPP_SpawnToy then return nil end
        if TPP_CanSpawn and not TPP_CanSpawn.Value then return nil end

        -- Do not spam SpawnToy. A missing/broken attachment is repaired by re-sticking
        -- the existing NinjaShuriken; a new one is spawned only after a short cooldown.
        local now = tick()
        if TPP_LastSpawn[plr.Name] and now - TPP_LastSpawn[plr.Name] < 0.75 then
            return nil
        end
        TPP_LastSpawn[plr.Name] = now

        local myRoot = TPP_GetRoot(TPP_LP)
        local targetPart = TPP_GetTargetPart(plr)
        if not myRoot or not targetPart then return nil end

        local folder = workspace:FindFirstChild(TPP_LP.Name .. "SpawnedInToys")
        local before = {}
        if folder then
            for _, obj in ipairs(folder:GetChildren()) do
                if obj.Name == "NinjaShuriken" then before[obj] = true end
            end
        end

        pcall(function()
            TPP_SpawnToy:InvokeServer("NinjaShuriken", myRoot.CFrame * CFrame.new(0, 3, 8), Vector3.zero)
        end)

        local shuriken
        local deadline = tick() + 3
        while tick() < deadline do
            folder = workspace:FindFirstChild(TPP_LP.Name .. "SpawnedInToys")
            if folder then
                for _, obj in ipairs(folder:GetChildren()) do
                    if obj.Name == "NinjaShuriken" and not before[obj] then
                        shuriken = obj
                        break
                    end
                end
            end
            if shuriken then break end
            task.wait(0.08)
        end
        if not shuriken then return nil end

        local sticky = shuriken:FindFirstChild("StickyPart") or shuriken:WaitForChild("StickyPart", 2)
        if not sticky then return nil end

        -- Keep the spawned Kunai non-interactive so other players cannot physically bump it.
        ...
        if TPP_SetOwner and shuriken:FindFirstChild("SoundPart") then
            pcall(function() TPP_SetOwner:FireServer(shuriken.SoundPart, shuriken.SoundPart.CFrame) end)
        end

        -- The toy is spawned by LocalPlayer; only its StickyPart is sent to the target part.
        if TPP_StickyEvent then
            pcall(function()
                TPP_StickyEvent:FireServer(
                    sticky,
                    targetPart,
                    CFrame.new(0, 0, 0.75) * CFrame.Angles(0, math.rad(90), math.rad(90))
                )
            end)
        end

        TPP_Shurikens[plr.Name] = shuriken
        return shuriken
    end

    local function TPP_EnsureKunai(plr)
        if not TPP_AntiKick or not TPP_Protected[plr.Name] then return end
        local targetPart = TPP_GetTargetPart(plr)
        if not targetPart then return end
        local toy = TPP_Shurikens[plr.Name]
        if not toy or not toy.Parent or not toy:FindFirstChild("StickyPart") then
            TPP_DestroyFor(plr)
            toy = TPP_SpawnFor(plr)
        end
        if toy and toy.Parent then
            ...
            local wantedPos = (targetPart.CFrame * CFrame.new(0, 0, 0.75)).Position
            local inPlace = sticky and ((sticky.Position - wantedPos).Magnitude <= 2)
            local attachedToTarget = weld and weld.Part1 == targetPart and inPlace
            if sticky and TPP_StickyEvent and not attachedToTarget then
                pcall(function()
                    TPP_StickyEvent:FireServer(sticky, targetPart, CFrame.new(0,0,0.75) * CFrame.Angles(0, math.rad(90), math.rad(90)))
                end)
            end
        end
    end

    local function TPP_GetAttacker(char, victim)
        if not char then return nil end
        for _, partName in ipairs({"Head", "HumanoidRootPart", "Torso", "UpperTorso"}) do
            ...
            if owner and owner:IsA("StringValue") and owner.Value ~= "" then
                ...
                if value ~= TPP_LP.Name and value ~= victim.Name then
                    return value
                end
            end
        end
    ...
    local function TPP_Rescue(plr)
        if not TPP_AntiGrab then return end

        local now = tick()
        if TPP_Handling[plr.Name] then return end
        if TPP_RescueCooldown[plr.Name] and now - TPP_RescueCooldown[plr.Name] < 0.15 then return end
        TPP_Handling[plr.Name] = true
        TPP_RescueCooldown[plr.Name] = now

        task.spawn(function()
            local myRoot = TPP_GetRoot(TPP_LP)
            local targetRoot = TPP_GetRoot(plr)
            if not myRoot or not targetRoot then
                TPP_Handling[plr.Name] = nil
                return
            end

            local distance = (myRoot.Position - targetRoot.Position).Magnitude

            -- If already inside the protection radius, stay there and do NOT teleport again.
            -- If far away, teleport to the protected player's side once.
            if distance > TPP_NearDistance then
                myRoot.CFrame = targetRoot.CFrame * CFrame.new(0, 0.5, 2.5)
                myRoot.AssemblyLinearVelocity = Vector3.zero
            end

            -- Break the active grab/ownership attempt without repeatedly moving the local player.
            pcall(function()
                if TPP_EndEarly then TPP_EndEarly:FireServer(targetRoot) end
                if TPP_SetOwner then TPP_SetOwner:FireServer(targetRoot, targetRoot.CFrame) end
                if TPP_DestroyLine then
                    TPP_DestroyLine:FireServer(targetRoot)
                    local head = plr.Character and plr.Character:FindFirstChild("Head")
                    if head then TPP_DestroyLine:FireServer(head) end
                end
            end)
    ...
    local TPP_Group = PlayerTab:Section({Text = "Third Party Protection"})

    local TPP_Dropdown
    TPP_Dropdown = TPP_Group:Dropdown({
        Text = "Select Target",
        Flag = "TPP_SelectedPlayer",
        List = TPP_GetPlayerList(),
        Callback = function(value)
            local username = tostring(value or ""):match("@([%w_]+)")
            if not username then return end
            if TPP_Protected[username] then
                TPP_Protected[username] = nil
                TPP_DestroyFor(TPP_Players:FindFirstChild(username))
            else
                TPP_Protected[username] = true
                local plr = TPP_Players:FindFirstChild(username)
                if plr and TPP_AntiKick then TPP_EnsureKunai(plr) end
            end
            pcall(function() TPP_Dropdown:Refresh({Text = "Select Target", List = TPP_GetPlayerList()}) end)
        end
    })

    TPP_Group:Button({Text = "Refresh Players", Callback = function()
        pcall(function() TPP_Dropdown:Refresh({Text = "Select Target", List = TPP_GetPlayerList()}) end)
    end})

    TPP_Group:Toggle({Text = "Anti Grab", Flag = "TPP_AntiGrab", Default = false, Callback = function(v)
        TPP_AntiGrab = v
    end})

    TPP_Group:Toggle({Text = "Anti-Kick Kunai", Flag = "TPP_AntiKickKunai", Default = false, Callback = function(v)
        TPP_AntiKick = v
        if v then
            task.spawn(function()
                while TPP_AntiKick do
                    for username in pairs(TPP_Protected) do
                        local plr = TPP_Players:FindFirstChild(username)
                        if plr then TPP_EnsureKunai(plr) end
                    end
                    task.wait(0.01)
                end
            end)
        else
            for username in pairs(TPP_Shurikens) do
                local plr = TPP_Players:FindFirstChild(username)
                if plr then TPP_DestroyFor(plr) end
            end
        end
    end})
    ...
    task.spawn(function()
        while true do
            task.wait(0.03)
            if TPP_AntiGrab then
                for username in pairs(TPP_Protected) do
                    local plr = TPP_Players:FindFirstChild(username)
                    if plr and plr.Character then
                        if TPP_GetAttacker(plr.Character, plr) then
                            TPP_Rescue(plr)
                        end
                    end
                end
...
-- Screen Other (XOCU logic, isolated from the Player scope)
do
    local playerScreenSec = PlayerTab:Section({Text = "Screen Other", Side = "Right"})
    local screenEnabled = false
    local screenIntensity = 0
    local screenConnection = nil

    local function setScreenEnabled(enabled)
            ...
            screenConnection = RunService.RenderStepped:Connect(function()
                ...
                if cam then
                    cam.CFrame = cam.CFrame * CFrame.new(
                        0, 0, 0,
                        1, 0, 0,
                        0, 0.65 + screenIntensity, 0,
                        0, 0, 1
                    )
                end
            end)
        elseif screenConnection then
            screenConnection:Disconnect()
            screenConnection = nil
        end
    end

    playerScreenSec:Toggle({
        Text = "Screen Stretch Effect",
        Flag = "PlayerScreenStretch",
        Default = false,
        Callback = function(v)
            setScreenEnabled(v)
        end
    ...
    playerScreenSec:Slider({
        Text = "Screen Intensity",
        Flag = "PlayerScreenStretchIntensity",
        Minimum = 0,
        Maximum = 20,
        Default = 0,
        ValueName = "",
...
do
    ...
    tkControlSec:Toggle({Text = "Stick to Grabbed Part", Flag = "BreaksStickToGrabbedPart", Default = false})
    tkControlSec:Toggle({Text = "Find Closest BaseGround", Flag = "BreaksFindClosestBaseGround", Default = false})
    tkControlSec:Toggle({Text = "Bring Train (Use vfly in IY)", Flag = "BreaksBringTrain", Default = false})

    end)()


-- Customize: transferred visual controls
local Visuals = {}
...
Visuals.AuraModels={
    ...
    function Visuals.updateHats() for c,h in pairs(Visuals.HatParts) do if h and h.Parent and c==LocalPlayer.Character then h.Transparency=Visuals.HatTransparency; h.Color=Visuals.HatRainbow and Color3.fromHSV((tick()%5)/5,1,1) or Visuals.HatColor end end end
    ...
    function Visuals.updateForceField() if not(LocalPlayer.Character and Visuals.ForceFieldEnabled) then return end; for _,p in ipairs(LocalPlayer.Character:GetDescendants()) do if p:IsA("BasePart") and p.Name~="Hat" and p.Material==Enum.Material.ForceField then p.Color=Visuals.ForceFieldRainbow and Color3.fromHSV((tick()%5)/5,1,1) or Visuals.ForceFieldColor end end end
    ...
    function Visuals.updateAuraLogic() local id=Visuals.CustomAuraID~="" and("rbxassetid://"..Visuals.CustomAuraID:gsub("%D","")) or Visuals.AuraModels[Visuals.AuraType]; if not id then return end; local ok,m=pcall(function() return game:GetObjects(id)[1] end); if ok and m then Visuals.CurrentAuraModel=m; if Visuals.AuraEnabled and LocalPlayer.Character then Visuals.enableAura(LocalPlayer.Character) end end end



local CustomizeTab = Window:Tab({Text="Customize"})
local customizeHairTrail = CustomizeTab:Section({Text="Hat & Trail", Side="Left"})
local customizeSkinAura = CustomizeTab:Section({Text="Skin & Aura", Side="Right"})

customizeHairTrail:Toggle({Text="Chinese Hat", Flag="CustomizeChineseHat", Default=false, Callback=function(v)
    Visuals.HatEnabled=v
    local c=LocalPlayer.Character
    if c then if v then Visuals.addHat(c) else Visuals.removeHat(c) end end
end})
customizeHairTrail:Toggle({Text="Rainbow Hat", Flag="CustomizeRainbowHat", Default=false, Callback=function(v) Visuals.HatRainbow=v end})
customizeHairTrail:Slider({Text="Hat Transparency", Flag="CustomizeHatTransparency", Minimum=0, Maximum=100, Default=30, ValueName="", Callback=function(v) Visuals.HatTransparency=v/100 end})
customizeHairTrail:Toggle({Text="Trail", Flag="CustomizeTrail", Default=false, Callback=function(v)
    Visuals.TrailEnabled=v
    local c=LocalPlayer.Character
    if c then if v then Visuals.addTrail(c) else Visuals.removeTrail(c) end end
end})
customizeHairTrail:Toggle({Text="Trail Gradient Mode", Flag="CustomizeTrailGradient", Default=false, Callback=function(v)
    Visuals.TrailGradient=v
    if Visuals.TrailEnabled and LocalPlayer.Character then Visuals.addTrail(LocalPlayer.Character) end
end})
customizeHairTrail:Toggle({Text="Trail Rainbow", Flag="CustomizeTrailRainbow", Default=false, Callback=function(v) Visuals.TrailRainbow=v end})
customizeHairTrail:Slider({Text="Trail Lifetime", Flag="CustomizeTrailLifetime", Minimum=1, Maximum=30, Default=5, ValueName="", Callback=function(v) Visuals.TrailLifetime=v/10 end})
customizeHairTrail:Slider({Text="Trail Transparency", Flag="CustomizeTrailTransparency", Minimum=0, Maximum=100, Default=0, ValueName="", Callback=function(v) Visuals.TrailTransparencyStart=v/100 end})

customizeSkinAura:Toggle({Text="ForceField Skin", Flag="CustomizeForceFieldSkin", Default=false, Callback=function(v)
    Visuals.ForceFieldEnabled=v
    local c=LocalPlayer.Character
    if c then if v then Visuals.applyForceField(c) else Visuals.removeForceField(c) end end
end})
customizeSkinAura:Toggle({Text="Rainbow ForceField", Flag="CustomizeRainbowForceField", Default=false, Callback=function(v) Visuals.ForceFieldRainbow=v end})
customizeSkinAura:Toggle({Text="Skin Trail", Flag="CustomizeSkinTrail", Default=false, Callback=function(v) Visuals.SkinTrailEnabled=v; Visuals.toggleSkinTrail(v) end})
customizeSkinAura:Slider({Text="Skin Trail Life", Flag="CustomizeSkinTrailLife", Minimum=1, Maximum=30, Default=5, ValueName="", Callback=function(v)
    Visuals.SkinTrailLife=v/10
    if Visuals.SkinTrailEnabled then Visuals.updateSkinTrail() end
end})
customizeSkinAura:Toggle({Text="Local Aura", Flag="CustomizeLocalAura", Default=false, Callback=function(v)
    Visuals.AuraEnabled=v
    local c=LocalPlayer.Character
    if v then
        if not Visuals.CurrentAuraModel then Visuals.updateAuraLogic() end
        if c then Visuals.enableAura(c) end
    else Visuals.disableAura() end
end})
local auraList={}
for k in pairs(Visuals.AuraModels) do table.insert(auraList,k) end
table.sort(auraList)
customizeSkinAura:Dropdown({Text="Aura Type", Flag="CustomizeAuraType", List=auraList, Callback=function(v)
    Visuals.AuraType=v; Visuals.CustomAuraID=""
    if Visuals.AuraEnabled then Visuals.updateAuraLogic() end
end})
-- MUSA UI has no native input control here; the selected Asset ID entry remains as a placeholder.
customizeSkinAura:Dropdown({Text="Custom Aura ID", Flag="CustomizeAuraID", List={"Asset ID"}, Callback=function(v)
    local id=tostring(v):gsub("%D","")
    if id~="" then Visuals.CustomAuraID=id; if Visuals.AuraEnabled then Visuals.updateAuraLogic() end end
end})
...
LocalPlayer.CharacterAdded:Connect(function(c)
    ...
    if Visuals.AuraEnabled then Visuals.enableAura(c) end
...
-- =========================================================
-- EKLENEN UI GÖRÜNÜM DÜZENLEMESİ
-- Daha açık gri arka plan + oval/capsule toggle ve butonlar
-- =========================================================
task.spawn(function()
    task.wait(0.5)
    ...
    local function addStroke(obj, color, thickness, transparency)
        if not obj:IsA("GuiObject") then return end
        local stroke = obj:FindFirstChild("CustomThemeStroke")
        if not stroke then
            stroke = Instance.new("UIStroke")
            stroke.Name = "CustomThemeStroke"
            stroke.Parent = obj
        end
        stroke.Color = color
    ...
    local function styleObject(obj)
        -- Ana panel / arka duvar
        if obj.Name == "Main" and obj:IsA("GuiObject") then
            obj.BackgroundColor3 = Color3.fromRGB(25, 28, 32)
            obj.BackgroundTransparency = 0
            addCorner(obj, UDim.new(0, 8))
        end

        -- Koyu panelleri biraz daha açık charcoal gri yap
        if obj:IsA("Frame") and obj.BackgroundTransparency < 1
            and obj.Name ~= "ToggleFrame"
            and obj.Name ~= "CheckIcon" then

            local c = obj.BackgroundColor3
            if c.R < 0.16 and c.G < 0.16 and c.B < 0.16 then
                obj.BackgroundColor3 = Color3.fromRGB(34, 38, 43)
                obj.BackgroundTransparency = 0
            end
        end

        -- Oval / capsule toggle dış gövdesi
        if obj.Name == "ToggleFrame" and obj:IsA("GuiObject") then
            obj.BackgroundColor3 = Color3.fromRGB(52, 56, 61)
            obj.BackgroundTransparency = 0
            addCorner(obj, UDim.new(1, 0))
            addStroke(obj, Color3.fromRGB(20, 22, 25), 2, 0.15)
        end

        -- Toggle içindeki yuvarlak kısım
        if obj.Name == "CheckIcon" and obj:IsA("GuiObject") then
            obj.BackgroundColor3 = Color3.fromRGB(92, 98, 105)
            obj.BackgroundTransparency = 0
            addCorner(obj, UDim.new(1, 0))
        end

        -- Normal butonlar
        if obj:IsA("TextButton") and obj.Name ~= "ToggleFrame"
            and obj.BackgroundTransparency < 1 then

            local c = obj.BackgroundColor3
            if c.R < 0.20 and c.G < 0.20 and c.B < 0.20 then
                obj.BackgroundColor3 = Color3.fromRGB(39, 43, 48)
                obj.BackgroundTransparency = 0
                addCorner(obj, UDim.new(0, 7))
                addStroke(obj, Color3.fromRGB(67, 73, 80), 1, 0.25)
            end
        end
    end
...
-- =========================================================
-- UI EK DÜZENLEME v2
-- Toggle'ları kare yerine dikdörtgen yap + sağ altta kullanıcı kartı
-- =========================================================
task.spawn(function()
    task.wait(0.75)
    ...
    local function rectangularToggle(obj)
        if not obj:IsA("GuiObject") then return end

        if obj.Name == "ToggleFrame" then
            -- Önceki tam oval görünümü kaldır: dikdörtgen, hafif yuvarlatılmış.
            local corner = obj:FindFirstChildOfClass("UICorner")
            if not corner then
                corner = Instance.new("UICorner")
                corner.Parent = obj
            ...
            local stroke = obj:FindFirstChild("RectToggleStroke")
            if not stroke then
                stroke = Instance.new("UIStroke")
                stroke.Name = "RectToggleStroke"
                stroke.Parent = obj
            end
            stroke.Color = Color3.fromRGB(18, 20, 23)
            stroke.Thickness = 1
            stroke.Transparency = 0.15
        elseif obj.Name == "CheckIcon" then
            -- İç parça yine yuvarlak kalsın.
            local corner = obj:FindFirstChildOfClass("UICorner")
            if not corner then
                corner = Instance.new("UICorner")
                corner.Parent = obj
...
-- =========================================================
-- AXUM HUB BAŞLIK DÜZELTME (ÖNCEKİ BAŞLIK ZORLA GİZLENİR)
-- =========================================================
task.spawn(function()
    task.wait(1)
    ...
    -- Eski oluşturulan başlıkları kaldır.
    for _, obj in ipairs(gui:GetDescendants()) do
        if obj.Name == "AXUMHeader" then
            obj:Destroy()
        end
    end
    ...
    local function replaceTitle()
        ...
        holder.Name = "AXUMHeader"
        holder.BackgroundTransparency = 1
        holder.BorderSizePixel = 0
        holder.Position = target.Position
        holder.AnchorPoint = target.AnchorPoint
        holder.Size = UDim2.new(0, 230, 0, 38)
        holder.ZIndex = target.ZIndex + 10
        ...
        logo.BackgroundTransparency = 1
        logo.BorderSizePixel = 0
        logo.Position = UDim2.new(0, 0, 0, 3)
        logo.Size = UDim2.new(0, 30, 0, 30)
        logo.ScaleType = Enum.ScaleType.Fit
        logo.ZIndex = holder.ZIndex + 1
        logo.Parent = holder

        -- Executor destekliyorsa yerel PNG'yi yükle.
        pcall(function()
            if getcustomasset then
                logo.Image = getcustomasset("images.png")
            elseif getsynasset then
                logo.Image = getsynasset("images.png")
            end
        end)
        ...
        title.Size = UDim2.new(0, 180, 0, 36)
        title.Font = Enum.Font.GothamBold
        title.Text = "AXUM HUB"
    ...
    -- GUI bazen geç oluştuğu için birkaç kez dene.
    for _ = 1, 20 do
        replaceTitle()
        if gui:FindFirstChild("AXUMHeader", true) then
            break
        end
        task.wait(0.15)
    end
...
task.spawn(function()
    ...
    local ASSET_ID = "rbxassetid://94670055497623"
    local logo = gui:FindFirstChild("AXUMLogo", true)
...
-- =========================================================
-- KULLANICI KARTINI EXPORT / IMPORT CONFIG ALTINA TAŞI
-- =========================================================
task.spawn(function()
    task.wait(2)
    ...
    local function findByText(wantedText)
        for _, obj in ipairs(gui:GetDescendants()) do
            if (obj:IsA("TextLabel") or obj:IsA("TextButton")) and obj.Text == wantedText then
                return obj
            end
        end
    end

    local card = gui:FindFirstChild("CheatUserCard", true)
    local importButton = findByText("Import Config")

    if card and importButton then
        -- Kartı Import Config ile aynı panelin içine al.
        local parent = importButton.Parent
        card.Parent = parent
        card.AnchorPoint = Vector2.new(0, 0)

        -- Import Config butonunun tam altına yerleştir.
        card.Position = UDim2.new(
            0,
            importButton.Position.X.Offset,
            0,
            importButton.Position.Y.Offset + importButton.Size.Y.Offset + 8
        )

        card.Size = UDim2.new(
            importButton.Size.X.Scale,
            importButton.Size.X.Offset,
            0,
            58
        )

        -- Panel rengini ana UI duvarıyla uyumlu tut.
        card.BackgroundColor3 = Color3.fromRGB(39, 43, 48)
        card.BackgroundTransparency = 0

        local stroke = card:FindFirstChildOfClass("UIStroke")
        if stroke then
            stroke.Color = Color3.fromRGB(67, 73, 80)
